...
require ['chart-modules/bar-groups-stacked/chart', 'chart-modules/pie/chart'], (barGroupsChart, pieChartMaker) ->
  ...
  d3.csv '/charts/wap-uas/data/wap-uas.csv', (data) ->
    ...
    pieData = [
      {name: 'Web', value: sum data.filter((d) -> !d.mobile)},
      {name: 'Mobile', value: sum data.filter((d) -> d.mobile)},
    ]

    pieChart = pieChartMaker().margin({right:120}).width(400)
    ...
    data = _.chain(data).groupBy((d) -> d.iso).map((arr,key) ->
      iso:key
      mobiles: sum arr.filter((d) -> d.mobile)
      total: sum arr
    ).value()

    chartData = data.filter((d) -> d.total > 100).map((d) ->
      name: d.iso.split('').filter((c,i) -> i<3).reduce ((a,b) -> a+b), ''
      values: [
        { name: 'mobile', value: d.mobiles, dev:1},
        { name: 'web', value: d.total-d.mobiles, dev: 1}
      ]
    )
